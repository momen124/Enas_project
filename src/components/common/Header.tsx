...
import logo from '/logo.png'; // Assuming in public/logo.png
...
const Header: React.FC = () => {
  ...
  return (
    <>
      <header className="bg-[var(--card-bg-color)] shadow-[0_1px_3px_var(--shadow-color)] relative z-50">
        {/* Top Bar */}
        <div className="bg-[var(--primary-color)] text-[var(--text-color)] py-2">
          <div className="max-w-7xl mx-auto px-4 flex justify-between items-center text-sm">
            ...
            <div className={`flex items-center space-x-4 ${isRTL ? 'space-x-reverse' : ''}`}>
              <button
                ...
                className="hover:text-[var(--hover-bg-color)] transition-colors"
              ...
              <select
                ...
                className="bg-transparent border-none text-[var(--text-color)] focus:outline-none focus:ring-2 focus:ring-[var(--border-color)] cursor-pointer"
              >
                {currencies.map((curr) => (
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            {/* Logo */}
            <Link href={`/${language}`} className="flex items-center space-x-2" title={t('home')}>
              <Image src={logo} alt="Cmon Elsonon logo" className="h-20 w-auto" width={80} height={80} />
              <span className="text-2xl font-bold text-[var(--primary-color)]">Cmon Elsonon</span>
            </Link>
            ...
            {/* Actions */}
            <div className={`flex items-center space-x-4 ${isRTL ? 'space-x-reverse' : ''}`}>
              <Link href={`/${language}/account`} className="p-2 hover:text-[var(--primary-color)] transition-colors" aria-label={t('account')}>
                <UserIcon className="w-6 h-6" />
              </Link>
              <Link href={`/${language}/wishlist`} className="p-2 hover:text-[var(--primary-color)] transition-colors" aria-label={t('wishlist')}>
              ...
              >
                <ShoppingBagIcon className="w-6 h-6 text-[var(--secondary-text-color)]" />
                {cartItemsCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-gold-accent-500 text-[var(--card-bg-color)] text-xs rounded-full w-5 h-5 flex items-center justify-center">
                    {cartItemsCount}
                  </span>
                )}
              </button>
              ...
              {categories.map((category) => (
                ...
                >
                  <Link
                    ...
                    className={`flex items-center py-4 px-2 text-[var(--text-color)] hover:text-[var(--primary-color)] transition-colors space-x-1 ${isRTL ? 'space-x-reverse' : ''}`}
                  >
                    ...
                    <div className="ps-6">
                      {category.children.map((subCategory) => (
                        <Link
                          ...
                          className="block px-4 py-2 text-[var(--secondary-text-color)] hover:bg-[var(--hover-bg-color)] hover:text-[var(--primary-color)] transition-colors"
