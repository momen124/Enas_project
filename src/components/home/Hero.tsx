...
import heroImage from '@/public/cmon.jpg'; // Assuming in public/
...
const Hero: React.FC = () => {
  ...
  return (
    <section
      ...
      style={{
        background: 'linear-gradient(to bottom right, var(--primary-color), var(--admin-primary))',
      }}
    >
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className={`${isRTL ? 'lg:order-2' : ''} animate-fade-in`}>
            <h1 className="text-4xl md:text-6xl font-bold mb-6 leading-tight tracking-wide text-[var(--text-color)] drop-shadow-lg">
              {t('heroTitle')}
            </h1>
            <p className="text-lg md:text-xl mb-8 text-[var(--secondary-text-color)]">
              {t('heroSubtitle')}
            </p>
            <div className={`flex space-x-4 ${isRTL ? 'space-x-reverse' : ''}`}>
              <Link
                ...
                className="bg-[var(--primary-color)] text-[var(--card-bg-color)] px-6 py-3 rounded-xl font-semibold hover:opacity-90 transition-all duration-300 inline-flex items-center space-x-2 shadow-md hover:shadow-lg"
              ...
              <Link
                ...
                className="border-2 border-[var(--border-color)] text-[var(--text-color)] px-6 py-3 rounded-xl font-semibold hover:bg-[var(--hover-bg-color)] transition-all duration-300 inline-flex items-center space-x-2 shadow-md hover:shadow-lg"
                aria-label="Learn more about us"
              >
                <span>Learn More</span>
              </Link>
            </div>
            <div className="mt-6 text-sm text-[var(--secondary-text-color)]">
          ...
          <div className={`${isRTL ? 'lg:order-1' : ''} animate-zoom-in`}>
            <div className="relative group">
              <Image
                ...
                height={320}
              />
              <div className="absolute -bottom-8 -right-8 bg-[var(--primary-color)] text-[var(--card-bg-color)] p-5 rounded-xl shadow-2xl animate-slide-up">
