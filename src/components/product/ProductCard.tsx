  ...
  return (
    <div className="group relative bg-[var(--card-bg-color)] rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300 border border-[var(--border-color)]">
      <Link href={`/${language}/product/${product.id}`}>
        ...
        <div className="p-4">
          <div className="text-xs text-[var(--primary-color)] font-medium mb-1">
            ...
            <div className="flex items-center gap-1 mb-2">
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <StarIcon
                    key={i}
                    className={`h-3 w-3 ${i < Math.floor(product.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-[var(--secondary-text-color)]'}`}
                  />
                ))}
              </div>
